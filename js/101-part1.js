...
// var onMarsRightNow; = value; false
...
//what is an array; create a variable;assign;values will be the list of fruits of strings
...
//add a string; where? end of an array
...
//first fruits then veggies to fruitandveggies
...
function quotient(number,numberTwo){
    return Math.floor(divide(number,numberTwo));
}
...
function remainder (number,numberTwo){
    return number % numberTwo;
}
...
function sumOfSquares(number,numberTwo){
    return add(square(number),square(numberTwo));
}
...
function areaOfRectangle(number,numberTwo){
